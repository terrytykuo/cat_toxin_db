...
const PLANTS_DIR = path.join(REPO_ROOT, 'data', 'plants_processed');
const FOODS_DIR = path.join(REPO_ROOT, 'data', 'foods_processed');
const OUTPUT_FILE = path.join(REPO_ROOT, 'data', 'toxins_transformed.json');
...
function main() {
  ...
  function loadDir(dir, category) {
    if (!fs.existsSync(dir)) return;
    const files = fs.readdirSync(dir).filter(f => f.endsWith('.json'));
    for (const file of files) {
      try {
        results.push(transformFile(path.join(dir, file), category));
      } catch (e) {
        console.error('Error processing ' + category + ' ' + file + ':', e.message);
        errors++;
      }
    }
  }

  loadDir(PLANTS_DIR, 'plant');
  loadDir(FOODS_DIR, 'food');
